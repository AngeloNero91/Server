...
void CHARACTER::fishing()
{
	...
	if (GetActiveGrowthPet())
	{
		ChatPacket(CHAT_TYPE_INFO, "[LS;1433]");
		return;
	}

	if (IsPetHatchWindowOpen())
	{
		ChatPacket(CHAT_TYPE_INFO, "[LS;1434]");
		return;
	}

	if (IsPetChangeNameWindowOpen())
	{
		ChatPacket(CHAT_TYPE_INFO, "[LS;1435]");
		return;
	}
#endif
...
void CHARACTER::OnClick(LPCHARACTER pkChrCauser)
{
	...
	if (IsPC())
	{
		if (!CTargetManager::instance().GetTargetInfo(pkChrCauser->GetPlayerID(), TARGET_TYPE_VID, GetVID()))
		{
			if (GetMyShop())
			{
				...
				{
					pkChrCauser->ChatPacket(CHAT_TYPE_INFO, "[LS;1344]");
					return;
				}
#endif
				//PREVENT_TRADE_WINDOW
	...
	if (pkChrCauser->GetPetWindowType() == PET_WINDOW_ATTR_CHANGE || pkChrCauser->GetPetWindowType() == PET_WINDOW_PRIMIUM_FEEDSTUFF)
	{
		pkChrCauser->ChatPacket(CHAT_TYPE_INFO, "[LS;1349]");
		return;
	}
#endif
