...
ACMD(do_pet_convert)
{
	char petQuery[QUERY_MAX_LEN];
	snprintf(petQuery, sizeof(petQuery), "SELECT id,name,level,evolution,exp,expi,bonus0,bonus1,bonus2,skill0,skill0lv,skill1,skill1lv,skill2,skill2lv,duration,tduration FROM new_petsystem");

	std::unique_ptr<SQLMsg> pkPetMsg(DBManager::instance().DirectQuery(petQuery));
	SQLResult* pPetRes = pkPetMsg->Get();
	MYSQL_ROW petData;

	if (pPetRes->uiNumRows > 0)
	{ 
		while ((petData = mysql_fetch_row(pPetRes->pSQLResult)))
		{
			int col = 0;
			DWORD dwID = atoi(petData[col++]);

			// Lets get item's owner, vnum, socket1 related to this pet.
			// In case it doesn't exist, pet will be skipped.
			DWORD dwOwner;
			DWORD dwVnum = 0;
			DWORD dwSocket0 = 0;

			char itemQuery[QUERY_MAX_LEN];
			snprintf(itemQuery, sizeof(itemQuery), "SELECT owner_id, vnum, socket0 FROM item_copy where id=%d", dwID);
			std::unique_ptr<SQLMsg> pkItemMsg(DBManager::instance().DirectQuery(itemQuery));
			SQLResult* pItemRes = pkItemMsg->Get();
			if (pItemRes->uiNumRows <= 0)
				continue;

			MYSQL_ROW itemData = mysql_fetch_row(pItemRes->pSQLResult);

			int col2 = 0;
			dwOwner = atoi(itemData[col2++]);
			dwVnum = atoi(itemData[col2++]);
			dwSocket0 = atoi(itemData[col2++]);

			const char* c_szName = petData[col++];
			DWORD dwLevel = atoi(petData[col++]);
			DWORD dwEvolution = atoi(petData[col++]);
			DWORD dwEXP = atoi(petData[col++]);
			DWORD dwItemEXP = atoi(petData[col++]);
			DWORD dwHP = atoi(petData[col++]);
			DWORD dwDef = atoi(petData[col++]);
			DWORD dwSP = atoi(petData[col++]);
			DWORD dwSkill1 = atoi(petData[col++]);
			DWORD dwSkillLevel1 = atoi(petData[col++]);
			DWORD dwSkill2 = atoi(petData[col++]);
			DWORD dwSkillLevel2 = atoi(petData[col++]);
			DWORD dwSkill3 = atoi(petData[col++]);
			DWORD dwSkillLevel3 = atoi(petData[col++]);
			DWORD dwDuration = atoi(petData[col++]);
			DWORD dwMaxDuration = atoi(petData[col++]);
			...
			for (int i = 0; i < PET_SKILL_COUNT_MAX; ++i)
			{
				switch (i)
				{
					case 0:
					{
						aSkill[i].bSkill = dwSkill1;
						aSkill[i].bLevel = dwSkillLevel1;
						aSkill[i].dwCooltime = 0;

						if (!dwSkill1)
							aSkill[i].bLocked = true;
						else
					...
					case 1:
					{
						aSkill[i].bSkill = dwSkill2;
						aSkill[i].bLevel = dwSkillLevel2;
						aSkill[i].dwCooltime = 0;
					...
					case 2:
					{
						aSkill[i].bSkill = dwSkill3;
						aSkill[i].bLevel = dwSkillLevel3;
						aSkill[i].dwCooltime = 0;
			...
			t.dwID = dwID;
			t.dwOwner = dwOwner;
			t.dwVnum = dwVnum == 55002 ? dwSocket0 : dwVnum;
			t.bState = dwVnum == 55002 ? STATE_BAG : STATE_UPBRINGING;
			strlcpy(t.szName, c_szName, sizeof(t.szName));
			t.bSize = number(1, 3);
			t.dwLevel = dwLevel;
			t.bLevelStep = 0;
			t.bEvolution = ++dwEvolution; // Public version starts with 0, while this one with 1
			t.bType = bPetType;
			thecore_memcpy(t.aSkill, aSkill, sizeof(aSkill));
			t.dwHP = dwHP;
			t.dwSP = dwSP;
			t.dwDef = dwDef;
			t.dwHPApply = dwHPApply;
			t.dwSPApply = dwSPApply;
			t.dwDefApply = dwDefApply;
			t.dwAgeApply = 0;
			t.lExp = dwEXP;
			t.lItemExp = dwItemEXP;
			t.lBirthday = time(0);
			t.lEndTime = time(0) + dwDuration;
			t.lMaxTime = dwMaxDuration;


			// Update database with the new pet structure data
			{
				char szQuery[QUERY_MAX_LEN];
				size_t queryLen;
				queryLen = snprintf(szQuery, sizeof(szQuery),
					"REPLACE INTO growth_pet_copy SET "
					"id = %d, "
					"owner_id = %d, "
					"vnum = %d, "
					"state = %d, "
				...
				DBManager::instance().EscapeString(buf, sizeof(buf), (char*)t.aSkill, sizeof(t.aSkill));
				snprintf(szQuery + queryLen, sizeof(szQuery) - queryLen, "skill_level = '%s' ", buf);
				DBManager::instance().DirectQuery(szQuery);

				if (dwVnum == 55002)
				{
					snprintf(szQuery, sizeof(szQuery), "UPDATE item_copy SET "
						"socket0 = %d, "
						"socket1 = %d, "
						"socket2 = %d"
						", attrtype0=0, attrvalue0=0"
						", attrtype1=0, attrvalue1=0"
						", attrtype2=0, attrvalue2=0"
						", attrtype3=0, attrvalue3=0"
						", attrtype4=0, attrvalue4=0"
						", attrtype5=0, attrvalue5=0"
						", attrtype6=0, attrvalue6=0 WHERE id=%d",
						time(0) + 3600 * 24 * 30,	// 30 Days
						dwDuration,
						dwID,
						dwID
					);
