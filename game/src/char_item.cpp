...
bool CHARACTER::DoRefine(LPITEM item, bool bMoneyOnly)
{
	...
	{
		ChatPacket(CHAT_TYPE_INFO, "[LS;1464]");
		return false;
	}
#endif
...
#endif
{
	...
	{
#ifdef __WJ_PICKUP_ITEM_EFFECT__
		item->AddToCharacter(this, TItemPos(item->GetWindowInventoryEx(), cell), isHighLight);
#else
...
#endif
{
	...
	if (iEmptyCell != -1)
	{
		if (bMsg)
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("아이템 획득: %s"), item->GetClientName());
#ifdef __WJ_PICKUP_ITEM_EFFECT__
		item->AddToCharacter(this, TItemPos(item->GetWindowInventoryEx(), iEmptyCell), isHighLight);
#else
		item->AddToCharacter(this, TItemPos(item->GetWindowInventoryEx(), iEmptyCell));
#endif
		LogManager::instance().ItemLog(this, item, "SYSTEM", item->GetName());
