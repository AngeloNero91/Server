...
LPGROWTH_PET CGrowthPet::Summon(LPITEM pSummonItem, bool bIsResummon /* = false */)
{
	...
	if (!bIsResummon)
	{
		m_pOwner->ChatPacket(CHAT_TYPE_INFO, "[LS;1146;%s]", m_strName.c_str());
...
void CGrowthPet::Unsummon()
{
	if (IsSummoned())
	{
		...
		if (m_pOwner && m_pOwner->GetDesc())
		{
			...
			m_pOwner->ChatPacket(CHAT_TYPE_INFO, "[LS;1147;%s]", m_strName.c_str());
		}
	}
...
bool CGrowthPet::Update()
{
	...
	{
		bUnsummon = true;
		this->GetOwner()->ChatPacket(1, "bUnsummon 1");
	}
	else if (!m_pSummonItem)
	{
		bUnsummon = true;
		this->GetOwner()->ChatPacket(1, "bUnsummon 2");
	}
	else if (m_pSummonItem->GetOwner() != this->GetOwner())
	{
		bUnsummon = true;
		this->GetOwner()->ChatPacket(1, "bUnsummon 3");
	}
...
void CGrowthPet::ChangePetPoint(BYTE bIdx, int iVal, bool bAmount /* false */, bool bPacket /* = true */)
{
	switch (bIdx)
	{
		case POINT_UPBRINGING_PET_LEVEL:
		{
			...
			if(m_pOwner)
				m_pOwner->ChatPacket(CHAT_TYPE_INFO, "[LS;1148;%s;%d]", m_strName.c_str(), dwNewLevel);
...
void CGrowthPet::NameChange(const char* c_szName, LPITEM pChangeNameItem, LPITEM pUpBringingItem)
{
	...
	if (strlen(c_szName) < PET_NAME_MIN_SIZE)
	{
		...
		m_pOwner->ChatPacket(CHAT_TYPE_INFO, "[LS;1171]");
		return;
	}
	...
	if (CBanwordManager::Instance().CheckString(c_szName, strlen(c_szName)))
	{
		...
		m_pOwner->ChatPacket(CHAT_TYPE_INFO, "[LS;1161]");
		return;
	}
	...
	if (m_pOwner->GetGold() < iPrice)
	{
		...
		m_pOwner->ChatPacket(CHAT_TYPE_INFO, "[LS;1173]");
		return;
	}
...
void CGrowthPet::Feed(const TPacketCGPetFeed* feedPacket)
{
	...
	{
		m_pOwner->ChatPacket(CHAT_TYPE_INFO, "[LS;1486]");
		return;
	}
	...
	switch (feedPacket->index)
	{
		...
		case FEED_EXP_EVENT:
		{
			...
			m_pOwner->ChatPacket(CHAT_TYPE_INFO, "[LS;1175;%d]", dwTotalEXP);
		} break;
	...
	switch (bType)
	{
		case EXP_TYPE_MOB:
		{
			...
			if (dwCurrentMobEXP >= dwRequiredMobEXP)
			{
				if (bError != LEVELUP_ERROR_NONE)
				{
					...
					{
						m_pOwner->ChatPacket(CHAT_TYPE_INFO, "[LS;1168]");
						m_bItemExpMsg = true;
					}
			...
			if (dwCurrentItemEXP >= GetRequiredItemExp(dwPetLevel))
			{
				if (dwCurrentMobEXP + dwEXP >= dwRequiredMobEXP)
				{
					...
					if (bError != LEVELUP_ERROR_NONE)
					{
						...
						{
							m_pOwner->ChatPacket(CHAT_TYPE_INFO, "[LS;1196]");
							m_bMobExpMsg = true;
						}
		...
		case EXP_TYPE_ITEM:
		{
			...
			if (dwCurrentItemEXP >= dwRequiredItemEXP)
			{
				...
				m_pOwner->ChatPacket(CHAT_TYPE_INFO, "[LS;1157;%s]", m_strName.c_str());
...
void CGrowthPet::Revive(LPITEM pSummonItem, BYTE bType)
{
	...
	else
	{
		...
		m_pOwner->ChatPacket(CHAT_TYPE_INFO, "[LS;1151]");
	}
...
void CGrowthPet::SendEvolveMessage()
{
	...
	for (DWORD i = 1; i <= PET_EVOLUTION_4; ++i)
	{
		if (i == GetPetPoint(POINT_UPBRINGING_PET_EVOL_LEVEL))
		{
			...
			if (i < PET_EVOLUTION_3)
				m_pOwner->ChatPacket(CHAT_TYPE_INFO, "[LS;1169]");
			else if (i == PET_EVOLUTION_3)
				m_pOwner->ChatPacket(CHAT_TYPE_INFO, "[LS;1178]");
		}
	}
}
...
bool CGrowthPet::Evolve(const TPacketCGPetFeed* feedPacket)
{
	...
	{
		m_pOwner->ChatPacket(CHAT_TYPE_INFO, "[LS;1170]");
		return true;
	}
	...
	{
		m_pOwner->ChatPacket(CHAT_TYPE_INFO, "[LS;1179;%d]", arPetEvolutionTable[idx].dwLevel);
		return true;
	}
	...
	if (dwEvolution < PET_EVOLUTION_3)
	{
		...
		{
			m_pOwner->ChatPacket(CHAT_TYPE_INFO, "[LS;1181]");
			return true;
		}
	}
	else if (dwEvolution == PET_EVOLUTION_3)
	{
		...
		if (dwDayCount < PET_LAST_EVOL_MIN_DAY_COUNT)
		{
			m_pOwner->ChatPacket(CHAT_TYPE_INFO, "[LS;1180;%d]", PET_LAST_EVOL_MIN_DAY_COUNT);
			return true;
		}
	}
	...
	for (int i = 0; i < PET_EVOL_MAX_ITEM_COUNT; ++i)
	{
		...
		if (!bIsFound)
		{
			...
			if (lItem)
				// m_pOwner->ChatPacket(CHAT_TYPE_INFO, "[LS;1195;[IN;%s];%d]", lItem->szLocaleName, requiredItem.second);
				m_pOwner->ChatPacket(CHAT_TYPE_INFO, "[LS;1195;[IN;%d];%d]", requiredItem.first, requiredItem.second);
		}
	}
	...
	if (itemVec.size() == PET_EVOL_MAX_ITEM_COUNT)
	{
		...
		m_pOwner->ChatPacket(CHAT_TYPE_INFO, "[LS;1156;%s;%s]", m_strName.c_str(), arPetEvolutionTable[idx].szEvolutionName);
	}
...
void CGrowthPet::Evolve(BYTE dwEvolution)
{
	...
	if(GetPetPoint(POINT_UPBRINGING_PET_EVOL_LEVEL) > PET_EVOLUTION_1)
		m_pOwner->ChatPacket(CHAT_TYPE_INFO, "[LS;1156;%s;%s]", m_strName.c_str(), arPetEvolutionTable[dwEvolution - 2].szEvolutionName);
}
...
void CGrowthPet::AttrChange(LPITEM pSummonItem, LPITEM pAttrChangeItem)
{
	...
	m_pOwner->ChatPacket(CHAT_TYPE_INFO, "[LS;1350]");
...
void CGrowthPet::LearnSkill(BYTE bSlot, LPITEM pSkillBookItem)
{
	...
	for (const auto& skill : m_aSkill)
	{
		...
		{
			m_pOwner->ChatPacket(CHAT_TYPE_INFO, "[LS;1176]");
			return;
		}
	}
	...
	m_pOwner->ChatPacket(CHAT_TYPE_INFO, "[LS;1174]");
	...
	else if (pSkillProto->GetSkillType() == PET_SKILL_USE_TYPE_AUTO)
	{
		...
		switch (bSkillAffect)
		{
			case PET_SKILL_AFFECT_HEAL:
			{
				...
				m_pOwner->ChatPacket(CHAT_TYPE_INFO, "[LS;1177;%s;%d]", pSkillProto->szName, wSkillValue);
			} break;
			...
			case PET_SKILL_AFFECT_INVINCIBILITY:
			{
				...
				m_pOwner->ChatPacket(CHAT_TYPE_INFO, "[LS;1185;%s]", pSkillProto->szName);
			} break;

			case PET_SKILL_AFFECT_REMOVAL:
			{
				m_pOwner->RemoveBadAffect();
				m_pOwner->ChatPacket(CHAT_TYPE_INFO, "[LS;1186;%s]", pSkillProto->szName);
			} break;
			...
			case PET_SKILL_FEATHER:
			{
				...
				m_pOwner->ChatPacket(CHAT_TYPE_INFO, "[LS;1401;%s;%s]", m_pOwner->GetName() , m_strName.c_str());
			} break;
		}
	}
...
void CGrowthPetManager::EggHatch(LPCHARACTER ch, const char* c_szName, TItemPos Cell)
{
	...
	if (strlen(c_szName) < PET_NAME_MIN_SIZE)
	{
		...
		ch->ChatPacket(CHAT_TYPE_INFO, "[LS;1171]");
		return;
	}
	...
	if (CBanwordManager::Instance().CheckString(c_szName, strlen(c_szName)))
	{
		...
		ch->ChatPacket(CHAT_TYPE_INFO, "[LS;1161]");
		return;
	}
	...
	if (lItem)
		ch->ChatPacket(CHAT_TYPE_INFO, "[LS;1150;%s]", lItem->szLocaleName);
}
